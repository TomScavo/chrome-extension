...
async function getNextBtnEle() {
    ...
    console.log(currentWebsiteData.nextBtn, document.querySelector(currentWebsiteData.nextBtn));
    return document.querySelector(classNames[getHostName()] || currentWebsiteData.nextBtn || null);
}
...
function getFullscreenBtnEle() {
    ...
    if (!classNames[getHostName()]) return null;

    return document.querySelector(classNames[getHostName()]);
}
...
function getHostName() {
    if (isIframe()) {
        ...
        if (!url) return '';
        const arr = url.split('//');
        return arr.length === 1 ? url : arr[1];
    } else {
        return window.location.hostname;
    }
}

async function saveNextBtnInfo(e) {
    const btnEle = findValidElement(e.target);
    try {
        const nextBtnInfo = uniqueSelector(btnEle);
        console.log('nextBtnInfo', nextBtnInfo);
    ...
    if (alertEle) {
        ...
        `
            <div style="display: flex; justify-content: center; align-items: center; position: fixed; color: #333; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 20px 0px; background: #fff; z-index: 9999; top: 5%; margin-left: 50%; transform: translate(-50%, 0);">
                ...
                <span style="color: #333;">绑定成功！可在设置中进行修改。</span>
            </div>
        `;
        setTimeout(() => {
            removeAlertEle();
                ...
                if (!isIframe()) {
                    ...
                    `
                        <div style="display: flex; justify-content: center; align-items: center; position: fixed; color: #333; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 20px 0px; background: #fff; z-index: 9999; top: 5%; margin-left: 50%; transform: translate(-50%, 0);">
                            ...
                            <span style="color: #333;">3 秒即可绑定成功。</span>
